...
namespace PlantConveyor
{
    ...
    public class Core : MelonMod
    {
        ...
        {
            if (!DecreaseCountdownTimer() || !Board.Instance || !InGameUI.Instance)
                return;
        ...
        private void SetPlantCard(GameObject cardObject, int index)
        {
            int plantIndex = ((FirstCardIndex + index) % PlantIDs.Count + PlantIDs.Count) % PlantIDs.Count;
            CardUI component = cardObject.GetComponent<CardUI>();
        ...
        {
            // Reset cooldowns when switching cards
            if (IsSwitching)
            {
                // Cooldown reset will be handled when card UI is updated
            }
        }
    }
}
